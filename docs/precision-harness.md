...
- **Per-category `min_precision: 1.0` + `min_recall: 1.0`** for the three
  fully-labelled categories (`injection`, `dependency`, `secret`) — each is backed
  entirely by committed labelled fixtures (no unlabelled finding can lower their
  precision), so unlike totals precision these are meaningful per-category bars.
...
per-analyzer / per-category precision/recall of the fully-labelled scopes are.
