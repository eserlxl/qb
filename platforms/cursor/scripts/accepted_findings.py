...
Only the backtick-wrapped leading token of each such list item is read as an
accepted id, so surrounding prose, rationale text, and other sections never leak
in. An absent or empty register accepts nothing (fail-closed), so an unreviewed
finding can never silently satisfy the production gate's ``self_audit_clean``
conjunct. No secret value may appear in the register (the no-committed-secrets
guard holds over it like every other tracked file).
"""
...
# A list item (no leading indent) whose FIRST token is a backtick-wrapped id.
_ACCEPTED_ENTRY = re.compile(r"^-\s+`([^`]+)`")


def parse_accepted_ids(text: str) -> set:
    """Parse the register text into the set of accepted finding ids.

    Only list items under the ``## Accepted`` heading whose first token is a
    backtick-wrapped id are read; everything else (prose, rationale, indented
    examples, other sections) is ignored, so the register cannot accidentally
    accept an id merely named in passing.
    """
    accepted: set = set()
    in_section = False
    for line in text.splitlines():
        ...
        if in_section:
            ...
            if match:
                accepted.add(match.group(1).strip())
    return accepted
