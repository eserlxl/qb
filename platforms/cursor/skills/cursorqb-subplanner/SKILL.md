...
5. Validate ALL generated files (not a sample): run
   ...
   confirm the 13 sections in order, Turkish, full-path index references, no duplicate/gap
...
When invoked from the orchestrator, control returns to `cursorqb-planner` Gate 2 (the audit
approval). The natural next step is the `cursorqb-auditor` skill.
