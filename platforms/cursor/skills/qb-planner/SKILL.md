...
follow the **`qb-autopsy`** skill. It writes only `Planner-docs/Autopsy.md` (a 13-section technical
...
- Approve -> launch Step 3 via the **define-goal procedure above** (Canonical Step-3 goal text), then
  follow the **`qb-auditor`** skill until it produces `Planner-docs/Sub-Planning-Audit.md` and returns a status.
...
- **Gate fails on P0/P1** (`step4_blocked_by_high_severity_findings`) -> do not offer Step 4. Show the
  ...
  named sub-plan files (within `Planner-docs/`), then re-run the `qb-auditor` skill. Repeat until the
