...
- **BREAKING — planning-artifact identifiers renamed to plain English.** The Turkish
  `Faz` phase token and the misspelled `Planing` were standardized to `Phase` /
  `Planning` across the planner prompts, the read-only validator, and the docs:
  ...
  **Migration:** `Planner-docs/` directories generated by 0.4.x are not recognized by
  the 0.5.0 validator. Either re-run the workflow to regenerate them, or rename the
