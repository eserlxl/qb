...
Step 3 should run the bundled validator first and incorporate its findings into `Planner-docs/Sub-Planning-Audit.md`. When manually validating from a QB repository checkout, use:
...
```text
Use $qb to run Step 2 on the existing Planner-docs/Main-Planning.md.
```
