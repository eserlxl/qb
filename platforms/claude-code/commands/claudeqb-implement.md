---
name: claudeqb-implement
description: Run only the gated ClaudeQB Step 4 implementation for one READY sub-plan slice.
---

# ClaudeQB Implement

Invoke the `claudeqb-implementer` skill directly, without re-running Steps 1-3.
...
gate passes, delegate this step to the matching `claudeqb-implementer` subagent via the Task tool
(fallback: run the `claudeqb-implementer` skill in-session under the same goal contract), selects one
