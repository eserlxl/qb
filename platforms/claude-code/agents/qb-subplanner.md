---
name: qb-subplanner
description: Use for QB Step 2 - decompose every main phase in Planner-docs/Main-Planning.md into detailed per-phase sub-plans under Planner-docs/Phase-<n>-Plans/ plus Sub-Planning-Index.md. Invoked by the qb-planner orchestrator via the Task tool (or run in-session as fallback) and runs until ALL phases are covered - it does not stop after one phase. Only changes files under Planner-docs/.
