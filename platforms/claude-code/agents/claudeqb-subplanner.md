...
- **Objective:** For every main phase in `Planner-docs/Main-Planing.md`, create
  `Planner-docs/Faz-<n>-Plans/` with one or more `Faz<n>.<m>-<ascii-kebab-slug>.md` sub-plans that
...
- Optional supporting source: if `Planner-docs/Autopsy.md` exists (from Step 1.5), read it fully
  ...
  risk sections. It is **not** a replacement for `Main-Planing.md`; do not block Step 2 when it is
  absent.
...
1. Read `Main-Planing.md` fully and extract the ordered list of main phases (preserve their order).
   If `Planner-docs/Autopsy.md` exists, read it fully too and treat it as supporting feedback.
2. For each phase, in order, create its `Faz-<n>-Plans/` folder and its sub-plan files following the
...
4. Build large sub-plans incrementally (create the file, then add sections) rather than one
   oversized write. When every phase is covered, create/update `Sub-Planing-Index.md` with **full
