...
description: Step 2 of the ClaudeQB planning workflow. Use to decompose every main phase in Planner-docs/Main-Planing.md into detailed sub-plans under Planner-docs/Faz-<n>-Plans/ plus Sub-Planing-Index.md. Runs under an explicit goal contract until ALL phases are covered - it does not stop after one phase. Invoke from claudeqb-planner Gate 1 (the orchestrator delegates this step to the claudeqb-subplanner subagent), or run directly to (re)generate sub-plans. Only changes files under Planner-docs/.
---

# ClaudeQB Sub-Planner (Step 2)
...
- **Source of truth:** `Planner-docs/Main-Planing.md` in the user's active workspace (cwd).
- **Optional supporting source:** if `Planner-docs/Autopsy.md` exists (from Step 1.5), read it fully
  ...
  **not** a replacement for `Main-Planing.md`; do not block Step 2 when it is absent.
...
- **Objective:** For every main phase in `Planner-docs/Main-Planing.md`, create
  `Planner-docs/Faz-<n>-Plans/` with one or more `Faz<n>.<m>-<ascii-kebab-slug>.md` sub-plans
...
- **Success evidence (all must hold):**
  - `Planner-docs/Sub-Planing-Index.md` exists and references **every** sub-plan by its full
    relative path (`Planner-docs/Faz-<n>-Plans/Faz<n>.<m>-*.md`), never slug-only - the validator
    rejects slug-only references.
  - Every phase detected in `Main-Planing.md` has a matching `Faz-<n>-Plans/` folder.
  - Every phase folder has at least one conforming `Faz<n>.<m>-*.md` sub-plan.
...
This step is the autonomous body of work that the `claudeqb-planner` orchestrator delegates to the
`claudeqb-subplanner` subagent via the Task tool. The goal contract above is the subagent's task
...
1. Read `Main-Planing.md` fully and extract the ordered list of main phases (preserve their order). If
   `Planner-docs/Autopsy.md` exists, read it fully too and treat it as supporting feedback.
2. For each phase, in order, create its `Faz-<n>-Plans/` folder and its sub-plan files following
...
4. Build large sub-plans incrementally (create the file, then add sections) rather than one
   oversized write. When every phase is covered, create/update `Sub-Planing-Index.md` with
...
5. Validate ALL generated files (not a sample): run
   ...
   (fallback: `find Planner-docs -maxdepth 3 -type f | sort`, read every sub-plan's headings,
   confirm the 13 sections in order, English, full-path index references, no duplicate/gap
   numbering, and that only `Planner-docs/` changed). Fix every reported error before handing off.

## Blocked conditions (stop early, but leave a record)

Following `second-planner.md`: if `Planner-docs/Main-Planing.md` is missing, or it has no clear
phase roadmap, or repository read errors prevent safe planning, do NOT invent speculative
sub-plans. Instead create `Planner-docs/Step2-Blocked.md` explaining the blocker, the exact
...
When invoked from the orchestrator, control returns to `claudeqb-planner` Gate 2 (the audit
approval). The natural next step is the `claudeqb-auditor` skill.
