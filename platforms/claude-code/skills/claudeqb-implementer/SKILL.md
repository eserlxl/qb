---
name: claudeqb-implementer
description: Gated Step 4 of the ClaudeQB workflow - implement one bounded, reversible, testable slice from an audited plan. Use after the Step 3 audit when the user approves implementation, or via /claudeqb-implement. Runs only when the audit is PASS or PASS_WITH_WARNINGS with no P0/P1 findings; otherwise it stops and recommends repair. Selects a single READY sub-plan, defines the validation command first, makes a minimal change, and verifies before claiming done.
---

# ClaudeQB Implementer (Step 4, gated)

Implement one bounded, reversible slice from the audited ClaudeQB plan. This is the only
ClaudeQB step that changes source code, and it runs strictly behind the audit gate. Read
the full Step-4 spec in `fourth-planner.md` (next to this skill) and follow it end to end.

## Execution model

- **Delegated, in-session.** This step is run by the `claudeqb-implementer` subagent via the Task tool. Read the bundled `fourth-planner.md` spec and follow it under the slice goal contract (objective / success evidence / scope bounds / stop condition) passed as the subagent task brief.
...
- Any P0 or P1 finding -> do not implement; recommend the targeted repairs first (`claudeqb-auditor` fix list + `claudeqb-planner` repair loop); stop.
...
or the exact blocker. After a successful slice, the user can run `/claudeqb-implement`
