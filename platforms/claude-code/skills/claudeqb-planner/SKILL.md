...
description: Guided five-step (1, 1.5, 2, 3, 4), delegated project planning orchestrator with a repo-aware intake. Use when the user runs /claudeqb-plan or asks ClaudeQB to plan a project end to end - master plan, existing-project autopsy, phase decomposition, coverage audit, then optional implementation. Runs a repo-aware Step 1 intake, produces Planner-docs/Main-Planing.md, runs a Step 1.5 autopsy (claudeqb-autopsy) for existing projects, then gates into phase decomposition (claudeqb-subplanner), a coverage/quality audit (claudeqb-auditor), and a gated implementation step (claudeqb-implementer) - each long autonomous step delegated automatically to a matching subagent via the Task tool. Validates each step with the bundled validator.
---

# ClaudeQB Planner (orchestrator)

Drive the ClaudeQB planning workflow inside the current chat session, with an explicit
...
`claudeqb-autopsy`, `claudeqb-subplanner`, `claudeqb-auditor`, and `claudeqb-implementer` subagents
via the Task tool.

```text
Step 1   repo-aware intake -> First-Planner -> Main-Planing.md       (interactive)   [validator step1]
Step 1.5 claudeqb-autopsy  -> Autopsy.md (existing projects only)    (delegated)      [validator step2 autopsy]
 Gate 1 feedback (Main-Planing + Autopsy) + approve
Step 2   claudeqb-subplanner -> Faz-*-Plans/ + Sub-Planing-Index.md  (delegated)      [validator step2]
 Gate 2 audit approval
Step 3   claudeqb-auditor     -> Sub-Planing-Audit.md                (delegated)      [validator step3]
 Repair PASS_WITH_WARNINGS -> targeted fixes -> re-audit
 Step 4 gate: validator step4 (audit not BLOCKED, no P0/P1)
Step 4   claudeqb-implementer -> one reversible code slice           (delegated, gated)
```
...
- **Delegated steps use the Task tool, automatically.** Steps 1.5, 2, 3, and 4 are delegated to the
  matching `claudeqb-*` subagent via the Task tool, in-session and hands-free. There is no copy/paste
...
- **Never mutate the templates.** Do placeholder substitution in memory only. The bundled prompts
  (`planners/first-planner.md` and the specs inside `claudeqb-autopsy` / `claudeqb-subplanner` /
  `claudeqb-auditor` / `claudeqb-implementer`) must stay byte-for-byte pristine for the next run.
...
follow the **`claudeqb-autopsy`** subagent/skill. It writes only `Planner-docs/Autopsy.md` (a 13-section technical
...
1. Use the Task tool to spawn the matching subagent (`claudeqb-autopsy` for Step 1.5, `claudeqb-subplanner`
   for Step 2, `claudeqb-auditor` for Step 3, `claudeqb-implementer` for Step 4). Pass that step's **goal
   ...
   (`<plugin-root>/skills/claudeqb-autopsy/autopsy-planner.md`,
   `<plugin-root>/skills/claudeqb-subplanner/second-planner.md`,
   `<plugin-root>/skills/claudeqb-auditor/third-planner.md`, or
   `<plugin-root>/skills/claudeqb-implementer/fourth-planner.md`). The user has explicitly opted into this
...
- Step 1.5 (`claudeqb-autopsy`, spec `skills/claudeqb-autopsy/autopsy-planner.md`):
  > Run Step 1.5 Autopsy per the claudeqb-autopsy autopsy-planner.md spec. Read Planner-docs/Main-Planing.md and inspect the current repository read-only; create or update only Planner-docs/Autopsy.md (a 14-heading technical feedback report). Skip for an empty or nearly empty repository and do not create Autopsy.md. Do not modify source code or Main-Planing.md.
- Step 2 (`claudeqb-subplanner`, spec `skills/claudeqb-subplanner/second-planner.md`):
  > Run Step 2 per the claudeqb-subplanner second-planner.md spec. Read all main phases in Planner-docs/Main-Planing.md; if Planner-docs/Autopsy.md exists, read it fully as supporting feedback and account for it in the sub-plans. For each phase, create Faz-<n>-Plans folders under Planner-docs and Faz<n>.<m>-*.md detailed sub-plan files. Do not stop until all phases are covered. Change only files under Planner-docs.
- Step 3 (`claudeqb-auditor`, spec `skills/claudeqb-auditor/third-planner.md`):
  > Run Step 3 per the claudeqb-auditor third-planner.md spec. Audit Planner-docs/Main-Planing.md, Sub-Planing-Index.md, and all Faz-*-Plans/*.md files; analyze main-phase coverage, file naming, ordering, required section structure, index consistency, content quality, scope drift, readiness realism, security/governance, and Step 4 readiness. Do not fix any plan file; produce only Planner-docs/Sub-Planing-Audit.md. Do not stop until all phases and sub-plans are reviewed.
...
2. Ask the user (in their language) whether they have any feedback on the master plan and (if present) the
   ...
   (via the `claudeqb-autopsy` subagent/skill); re-validate (`--mode step1`, and `--mode step2` for the autopsy
...
3. Use `AskUserQuestion` to confirm proceeding to Step 2. Describe Step 2 plainly: "Read every main phase in
   ...
   - Decline -> stop gracefully; the user can resume with `/claudeqb-plan` or the `claudeqb-subplanner` skill.
   - Approve -> launch Step 2 via the **delegation procedure above** (Canonical Step-2 brief), then
     run the **`claudeqb-subplanner`** subagent/skill to completion across all phases.

## Gate 2 - approve Step 3 (audit)

After `claudeqb-subplanner` reports completion, present this confirmation, then ask with `AskUserQuestion`
(Yes / No):
...
- Decline -> stop gracefully; the user can run the audit later via the `claudeqb-auditor` skill or `/claudeqb-audit`.
- Approve -> launch Step 3 via the **delegation procedure above** (Canonical Step-3 brief), then
  run the **`claudeqb-auditor`** subagent/skill until it produces `Planner-docs/Sub-Planing-Audit.md` and returns a status.
...
- **Gate passes (PASS, or PASS_WITH_WARNINGS with no P0/P1)** -> summarize; surface any P2/P3 items from
  ...
  Step 4 via the delegation procedure and run the **`claudeqb-implementer`** subagent/skill for one READY sub-plan slice.
- **Gate fails on P0/P1** (`step4_blocked_by_high_severity_findings`) -> do not offer Step 4. Show the
  ...
  named sub-plan files (within `Planner-docs/`), then re-run the `claudeqb-auditor` subagent/skill. Repeat until the
