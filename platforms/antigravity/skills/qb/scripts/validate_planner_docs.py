...
# see references/project-comprehension-methods.md for the methodology.
...
def _mask_fenced_regions(text: str) -> str:
    """Blank the visible content of *balanced* fenced code blocks, preserving offsets.

    Heading detection runs over the masked text so a heading-like line such as
    ``## 13. ...`` quoted inside a code fence is not mistaken for a real document
    heading; bodies are still sliced from the original text by the same offsets. An
    unterminated fence is left literal so a dangling fence cannot hide a real heading.
    """
...
def validate_step4_readiness(state: ValidationState) -> None:
    ...
    findings = [
        (fid, severity, "open")
        for fid, severity in AUDIT_FIX_RE.findall(markdown_section(text, "## 13. Priority Fix List"))
    ]
    validate_readiness_rows(parse_readiness_rows(text), audit_path, state, findings)
...
def validate_audit_section_depth(text: str, path: Path, state: ValidationState) -> None:
    """Warn when an audit section is headings-only; an error under --strict."""
...
def parse_readiness_rows(text: str) -> list[dict[str, str]]:
    """Return the Step-4 readiness table rows, or [] when the table is absent."""
...
def run_validation(root: Path, mode: str, strict: bool = False) -> int:
    ...
    if mode in {"assessment", "step2", "step3", "step4", "all"}:
